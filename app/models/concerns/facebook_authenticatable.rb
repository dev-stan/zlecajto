...
module FacebookAuthenticatable
  ...
  class_methods do
    ...
    def from_facebook(auth)
      ...
      if user.present?
        user.link_facebook_auth(auth)
      else
        user = create_user_from_facebook(auth)
        user.attach_profile_picture_from_facebook(auth)
      end
    ...
    def create_user_from_facebook(auth)
      create(
